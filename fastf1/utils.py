...
CACHE_PATH =  os.path.expanduser('~') + '/Documents/FF1Data'
CACHE_ENABLE = True
"""Boolean: Enable/Disable cache for parsed data (Everything under
cache path). Note that raw http requests are always cached. Data is
...
def delta_time(reference_lap, compare_lap):
    ...
    def mini_pro(stream):
        ...
        return np.concatenate([[stream[0] - dstream_start], stream,
                               [stream[-1] + dstream_end]])
